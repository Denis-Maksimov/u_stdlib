...
static int is_metacharacter(const unsigned char *s) {
  ...
  return u_strchr(metacharacters, *s) != NULL;
}
...
static int toi(int x) {
  return u_isdigit(x) ? x - '0' : x - 'W';
}

static int hextoi(const unsigned char *s) {
  return (toi(u_tolower(s[0])) << 4) | toi(u_tolower(s[1]));
}
...
static int match_op(const unsigned char *re, const unsigned char *s,
  ...
  switch (*re) {
    case '\\':
      /* Metacharacters */
      switch (re[1]) {
        case 'S': FAIL_IF(u_isspace(*s), SLRE_NO_MATCH); result++; break;
        case 's': FAIL_IF(!u_isspace(*s), SLRE_NO_MATCH); result++; break;
        case 'd': FAIL_IF(!u_isdigit(*s), SLRE_NO_MATCH); result++; break;
    ...
    default:
      if (info->flags & SLRE_IGNORE_CASE) {
        FAIL_IF(u_tolower(*re) != u_tolower(*s), SLRE_NO_MATCH);
      } else {
  ...
  while (len <= re_len && re[len] != ']' && result <= 0) {
    ...
    if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
      ...
      result = info->flags &  SLRE_IGNORE_CASE ?
        u_tolower(*s) >= u_tolower(re[len]) && u_tolower(*s) <= u_tolower(re[len + 2]) :
        *s >= re[len] && *s <= re[len + 2];
      len += 3;
    } else {
  ...
  for (i = 0; i < re_len; i += step) {
    ...
    if (re[i] == '|') {
      ...
      if (re[i + 1] == 'x') {
        ...
        FAIL_IF(re[i + 1] ==  'x' && !(u_isxdigit(re[i + 2]) &&
                u_isxdigit(re[i + 3])), SLRE_INVALID_METACHARACTER);
      } else {
...
int slre_match(const char* regexp, const char* s, int s_len, struct slre_cap* caps, int num_caps, int flags)
{
    ...
    return foo(regexp, (int) u_strlen(regexp), s, s_len, &info);
}
